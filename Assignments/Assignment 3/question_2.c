...
void getData(struct stack *s)
{
    ...
    else
    {
        ...
        {
            printf("Roll: %d, Name: %s, Maths: %d, English: %d, Total Marks: %d, Rank: %d\n",
                   s->arr[i].roll, s->arr[i].std_name, s->arr[i].maths_marks, s->arr[i].english_marks,
                   s->arr[i].total_marks, s->arr[i].rank);
        }
    }
}
